...
from flowhound.vulnerabilities.io.version_detection import convert_version_to_tuple, convert_tuple_to_version, get_target_version
...
@pytest.mark.parametrize("target_version, error, error_msg", convert_version_to_tuple_invalid)
...
@pytest.mark.parametrize("target_version, error, error_msg", convert_tuple_to_version_invalid)
...
def test_get_target_version_success():
    ...
    with patch('flowhound.vulnerabilities.io.version_detection.get', return_value=mock_resp):
...
def test_get_target_version_non_200_raises_runtime_error():
    ...
    with patch('flowhound.vulnerabilities.io.version_detection.get', return_value=mock_resp):
        with pytest.raises(RuntimeError, match='Unable to retrieve Langflow version'):
...
def test_get_target_version_network_error_raises_runtime_error():
    with patch('flowhound.vulnerabilities.io.version_detection.get', side_effect=ConnectionError('refused')):
        with pytest.raises(RuntimeError, match='Error retrieving target Langflow version'):
            get_target_version(base_url='http://localhost:7860')
